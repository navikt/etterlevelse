...
public interface EtterlevelseDokumentasjonRepo extends JpaRepository<EtterlevelseDokumentasjon, UUID> {
    ...
    @Query(value = "SELECT * FROM etterlevelse_dokumentasjon WHERE data -> 'ardoqSystemIds' @> '[]'::jsonb AND data -> 'ardoqSystemIds' != '[]'::jsonb", nativeQuery = true)
    ...
    @Query(value = "select * from etterlevelse_dokumentasjon where data ->> 'nomAvdelingId' = ?1 or (?1 = '' and (data -> 'nomAvdelingId' is null or (data -> 'nomAvdelingId' IS NOT NULL AND data ->> 'nomAvdelingId' IS NULL)))", nativeQuery = true )
