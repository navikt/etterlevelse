      ...
      <Block display="flex" justifyContent="space-between" marginBottom=".5rem">
        ...
        <Block width="300px" display="flex" justifyContent="space-between">
          ...
          <CustomizedStatefulSelect
            ...
            onChange={p => setTable(p?.value[0]?.id as ObjectType)}
          />
        </Block>
      </Block>
          ...
          tableData.data.map((audit, index) => {
            ...
            return (
              <Row key={audit.id}>
                ...
                <Cell>
                  <StatefulTooltip
                    content={audit.tableId}
                    placement={PLACEMENT.top}
                  >
                    <Block color={colors[audit.tableId]}>
...
const tableId = (audit: AuditItem) => {
  if (audit.table === ObjectType.Codelist)
    return audit.tableId.substr(0, audit.tableId.indexOf('-'))
  if (audit.table === ObjectType.BehandlingData)
    return (audit.data as any).data.behandlingId
