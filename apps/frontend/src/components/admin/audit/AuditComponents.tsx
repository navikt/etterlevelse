...
export const AuditActionIcon = (props: {
  action: AuditAction
  withText?: boolean
}) => {
  const icon = (props.action === AuditAction.CREATE && {
    icon: faPlusCircle,
    color: theme.colors.positive300,
  }) ||
    (props.action === AuditAction.UPDATE && {
      icon: faInfoCircle,
      color: theme.colors.warning300,
    }) ||
    (props.action === AuditAction.DELETE && {
      icon: faMinusCircle,
