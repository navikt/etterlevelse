...
export const loginUrl = (history: History, path?: string) => {
  const frontpage = window.location.href.substr(
    0,
    window.location.href.length - history.location.pathname.length,
  )
  ...
  const roller = (
    <Block>
      <Button
        ...
        icon={viewRoller ? faChevronUp : faChevronDown}
      >
        Endre aktive roller
      </Button>
      <Block
        display={viewRoller ? 'block' : 'none'}
      ...
      >
        {user.getAvailableGroups().map(g => (
          <Checkbox
            ...
            onChange={e =>
              user.toggleGroup(g.group, (e.target as HTMLInputElement).checked)
            }
...
const Divider = (props: {compact?: boolean}) => (
  <Block
    minHeight={props.compact ? '5px' : '20px'}
    display={'flex'}
    flexDirection={'column'}
    justifyContent={'center'}
  >
    <Block
      $style={{borderBottom: '1px solid ' + ettlevColors.green50}}
      width={'100%'}
    />
  </Block>
)
  ...
  return (
    <Block display={'flex'} marginBottom={theme.sizing.scale600}>
      ...
      <Block alignSelf={'flex-end'} marginLeft={theme.sizing.scale800}>
        <StyledLink href={`/logout?redirect_uri=${frontpage}`}>
          Logg ut
        </StyledLink>
      </Block>
    </Block>
          ...
          {allPages.map((p, i) => {
            const item =
              !!p.href && !p.disabled ? (
                ...
                <Block $style={{opacity: p.disabled ? 0.6 : 1}}>
                  {p.label}
                </Block>
              )
            return (
  ...
  return (
    <Block width="100%" maxWidth={maxPageWidth}>
      <Block
        ...
        height="76px"
        overrides={{
          Block: {props: {role: 'banner', 'aria-label': 'Header meny'}},
        }}
      >
        <HeaderNavigation
          overrides={{
            Root: {style: {paddingBottom: 0, borderBottomStyle: 'none'}},
          }}
        >
            ...
            {!props.noSearchBar && (
              <NavigationItem>
                <Block
                  ...
                  overrides={{Block: {props: {role: 'search'}}}}
                >
                  <MainSearch />
                </Block>
              </NavigationItem>
