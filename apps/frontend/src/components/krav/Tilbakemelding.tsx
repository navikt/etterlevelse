...
export const Tilbakemeldinger = ({krav}: {krav: Krav}) => {
  const [tilbakemeldinger, loading, add, replace] = useTilbakemeldinger(
    krav.kravNummer,
    krav.kravVersjon,
  )
  const [focusNr, setFocusNr] = useState<string | undefined>(
    useQueryParam('tilbakemeldingId'),
  )
  ...
  const setFocus = (id: string) => {
    setFocusNr(id)
    history.replace(
      `/krav/${krav.kravNummer}/${krav.kravVersjon}?tilbakemeldingId=${id}`,
    )
  }
    ...
    <Block width="100%">
                    ...
                    >
                      <Block display={'flex'} justifyContent={'space-between'}>
                        <Block>
                          ...
                          <ParagraphSmall marginTop={0} marginBottom={0}>
                            {moment(t.meldinger[0].tid).format('ll')}:{' '}
                            {typeText(t.type)}
                          </ParagraphSmall>
                        </Block>
                        ...
                        >
                          <ParagraphSmall marginTop={0} marginBottom={0}>
                            {t.meldinger.length} melding
                            {t.meldinger.length > 1 ? 'er' : ''}
                          </ParagraphSmall>
                          <ParagraphSmall marginTop={0} marginBottom={0}>
                            {ubesvart
                              ? 'Ubesvart'
                              : `Sist besvart ${moment(sistMelding.tid).format(
                                  'll',
                                )}`}
                          </ParagraphSmall>
                        </Block>
                      ...
                      <Block
                        ...
                        $style={{
                          borderTop:
                            focused && t.meldinger.length === 1
                              ? `1px solid ${ettlevColors.green50}`
                              : undefined,
                        }}
                      >
                        {/* meldingsliste */}
                        ...
                        >
                          <Block>
                            <Button
                              ...
                              hidePadding
                              icon={focused ? faChevronUp : faChevronDown}
                            >
                          ...
                          {melderOrKraveier && user.canWrite() && focused && (
                            <Block>
                              <Button
                                kind={
                                  ubesvartOgKraveier ? 'primary' : 'outline'
                                }
                                size={'compact'}
                                onClick={() => {
                                  setFocusNr(t.id)
      ...
      <Block marginTop={theme.sizing.scale1000}>
        ...
        <ParagraphMedium maxWidth={'600px'}>
          Gi tilbakemelding til kraveier dersom det er uklarheter vedrørende
        ...
        {user.canWrite() && (
          <Button
            kind={'primary'}
            size="compact"
            onClick={() => setAddTilbakemelding(true)}
          >
            Ny tilbakemelding
          </Button>
        )}
        {!user.canWrite() && (
          <ParagraphSmall>
            Logg inn på toppen for å legge inn tilbakemelding
          </ParagraphSmall>
        )}
      </Block>
...
const ResponseMelding = (props: {
  ...
  const sisteMelding =
    m.meldingNr ===
    tilbakemelding.meldinger[tilbakemelding.meldinger.length - 1].meldingNr
  ...
  return (
    <Block alignSelf={'flex-end'}>
      <ParagraphSmall marginBottom={0}>
        Sist endret av <PersonName ident={props.melding.endretAvIdent} /> -{' '}
              ...
              <ModalBody>
                <Block>
                  ...
                  <OptionField
                    ...
                    name="type"
                    clearable={false}
                    options={Object.values(TilbakemeldingType).map(o => ({
                      id: o,
                      label: typeText(o),
                    }))}
                  />
                      ...
                      >
                        <Block>
                          ...
                          >
                            ...
                            {adresseType !== AdresseType.SLACK &&
                              !values.varslingsadresse && (
                                <Button
                                  ...
                                  icon={faSlackHash}
                                  onClick={() =>
                                    setAdresseType(AdresseType.SLACK)
                                  }
                                >
                                  Slack-kanal
                                </Button>
                            ...
                            {adresseType === AdresseType.SLACK_USER && (
                            ...
                            {adresseType !== AdresseType.SLACK_USER &&
                              !values.varslingsadresse && (
                                <Button
                                  ...
                                  icon={faUser}
                                  onClick={() =>
                                    setAdresseType(AdresseType.SLACK_USER)
                                  }
                                >
                                  Slack-bruker
                                </Button>
                            ...
                            {adresseType !== AdresseType.EPOST &&
                              !values.varslingsadresse && (
                                <Button
                                  ...
                                  icon={faEnvelope}
                                  onClick={() =>
                                    setAdresseType(AdresseType.EPOST)
                                  }
                                >
                                  Epost
                                </Button>
                ...
                <Block display="flex" justifyContent="flex-end">
                  <Block>
                    {error && (
                      <Notification
                        kind="negative"
                        overrides={{Body: {style: {marginBottom: '-25px'}}}}
                      >
                        {error}
                      </Notification>
                    )}
...
const tilbakeMeldingStatus = (tilbakemelding: Tilbakemelding) => {
  const sistMelding =
    tilbakemelding.meldinger[tilbakemelding.meldinger.length - 1]
  ...
  const rolle =
    tilbakemelding?.melderIdent === user.getIdent()
      ? TilbakemeldingRolle.MELDER
      : TilbakemeldingRolle.KRAVEIER
...
const MeldingKnapper = (props: {
  ...
  if (
    (!user.isAdmin() && melding.fraIdent !== user.getIdent()) ||
    !user.canWrite()
  )
    return null
      ...
      {deleteModal && (
        ...
        >
          ...
          <ModalBody>
            {meldingNr === 1 && (
              <ParagraphMedium>
                Hele meldingstråden vil bli slettet.
              </ParagraphMedium>
            )}
            <ParagraphSmall>
        ...
        {user.isKraveier() && !loading && melderInfo.melder && (
          ...
          >
            ...
            <Button
              ...
              onClick={() =>
                setReplyRole(
                  replyRole === TilbakemeldingRolle.MELDER
                    ? TilbakemeldingRolle.KRAVEIER
                    : TilbakemeldingRolle.MELDER,
                )
              }
...
const newTilbakemelding = (
  krav: Krav,
): Partial<CreateTilbakemeldingRequest> => ({
...
const createTilbakemeldingSchema: yup.SchemaOf<CreateTilbakemeldingRequest> =
  yup.object({
    ...
    type: yup
      .mixed()
      .oneOf(Object.values(TilbakemeldingType))
      .required(required),
