        ...
        {(p: FieldArrayRenderProps) => {
          ...
          return (
            <>
              ...
              >
                <Block>
                  <Block marginBottom={theme.sizing.scale400}>
                    ...
                    >
                      <span>
                        <FontAwesomeIcon icon={faSlackHash} /> Legg til
                        slack-kanal
                      </span>
                    </Button>
                    <Button
              ...
              >
                <SlackChannelSearch
                  added={(p.form.values as Krav).varslingsadresser}
                  add={push}
                  close={() => setAddSlackChannel(false)}
                />
              </AddModal>
    ...
    <RenderTagList
      ...
      list={varslingsadresser.map((v, i) => {
        if (v.type === AdresseType.SLACK) {
          ...
          return (
            <Block key={i}>
              {channel ? slackChannelView(channel) : `Slack: ${v.adresse}`}
            </Block>
          )
          ...
          return (
            <Block key={i}>
              {user ? `Slack: ${user.name}` : `Slack: ${v.adresse}`}
            </Block>
          )
        }
...
export const SlackChannelSearch = ({
  added,
  add,
  close,
}: AddVarslingsadresseProps) => {
  ...
  return (
    <CustomizedStatefulSelect
      ...
      options={slackSearch.filter(
        ch => !added || !added.find(va => va.adresse === ch.id),
      )}
      onChange={({value}) => {
    ...
    <Block display="flex" flexDirection="column">
      <Block display="flex">
        ...
        <Block>
          <Button
            type="button"
            onClick={() => addEmail(user.getEmail())}
            marginLeft
          >
            Meg{' '}
          </Button>
...
export const slackChannelView = (channel: SlackChannel, long?: boolean) =>
