...
export const Chart = (props: ChartProps) => {
  ...
  const expData: ChartDataExpanded[] = data.map((d, idx) => {
    ...
    const colorIndex =
      data.length - 1 === colors.length && idx >= data.length - 1
        ? idx + 1
        : idx
    const pieData = {
      ...d,
...
const Visualization = (props: VisualizationProps) => {
  ...
  return (
    <Block position="relative">
      ...
      >
        <div onMouseLeave={() => setHover(-1)}>
          ...
          >
            ...
            {!data.length && (
              <Block width={size * 2 + 'px'} height={size * 2 + 'px'} />
            )}
            <Block
            ...
            >
              ...
              {data.map((d, idx) => (
                ...
                >
                  <Block
                    backgroundColor={
                      idx === hover ? theme.colors.accent50 : theme.colors.white
                    }
                    $style={cursor}
                    display="flex"
                    alignItems="center"
      ...
      >
        <StatefulTooltip
          content={type === 'bar' ? 'Kakediagram' : 'Søyledriagram'}
        >
    ...
    >
      ...
      {_.range(0, 11).map(i => (
        <React.Fragment key={i}>
          ...
          <path
            d={`M 80 ${100 + i * 100} l 1010 0 l 0 -1 l -1010 0 `}
            fill="black"
          />
        </React.Fragment>
      ))}
...
const Wedge = (props: PartProps) => {
  ...
  A ${scale} ${scale} 0 ${size >= 0.5 ? 1 : 0} 1 ${
    Math.cos((start + size) * tau) * scale
