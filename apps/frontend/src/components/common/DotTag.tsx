...
export const DotTag = (props: {children: ReactNode}) => (
  ...
  >
    <FontAwesomeIcon
      icon={faCircle}
      color={ettlevColors.black}
      style={{fontSize: '.45rem', marginTop: '9px'}}
      aria-hidden={true}
    />
...
const Content = (props: {
  ...
  if (list) {
    ...
    if (linkCodelist)
      return (
        <RouteLink
          href={urlForObject(list as ListName & NavigableItem, itemString)}
        >
...
export const DotTags = (props: DotTagsParams) => {
  ...
  return (
    <Block display={props.inColumn ? 'block' : 'flex'} flexWrap>
      {items.map((item, i) => (
        <Block
          marginBottom={props.inColumn ? theme.sizing.scale200 : 'none'}
          key={i}
          marginRight={
            i < items.length && !commaSeparator ? theme.sizing.scale200 : 0
          }
        >
          <DotTag>
            {' '}
