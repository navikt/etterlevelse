...
type TableContextType<T, K extends keyof T> = TableProps<T, K> & {
  tableState: TableState<T, K>
}
const createTableContext = _.once(<T, K extends keyof T>() =>
  React.createContext<TableContextType<T, K>>({} as TableContextType<T, K>),
)
const useTableContext = <T, K extends keyof T>() =>
  useContext(createTableContext<T, K>())
...
export const Table = <T, K extends keyof T>(props: TableProps<T, K>) => {
  ...
  const show = (col?: K) =>
    !col ||
    !props.config?.exclude ||
    !((props.config?.exclude || [])?.indexOf(col) >= 0)
      ...
      {!!props.config?.pageSizes && (
        ...
        >
          ...
          <Pagination
            ...
            labels={{
              nextButton: intl.nextButton,
              preposition: 'av',
              prevButton: intl.prevButton,
            }}
          />
        </Block>
      )}
...
export const Row = (props: RowProps) => {
  ...
  const styleProps: StyleObject = {
    ...
    borderLeftWidth:
      props.infoRow || props.selectedRow ? theme.sizing.scale300 : '0',
    borderLeftStyle: 'solid',
    ':hover': {
      backgroundColor:
        tableProps.hoverColor ||
        (props.infoRow ? theme.colors.mono100 : theme.colors.primary50),
    },
    ...props.$style,
  }
...
const PlainHeadCell = withStyle(
  StyledHeadCell,
  headerCellOverride.HeadCell.style,
)
...
const HeadCell = <T, K extends keyof T>(props: HeadProps<T, K>) => {
  ...
  const filterConf = tableProps.config?.filter
    ? tableProps.config.filter[column]
    : undefined
  const filterButton = filterConf && (
    ...
    >
      <FontAwesomeIcon
        ...
        icon={faFilter}
        color={
          !!inputFilter ? theme.colors.negative400 : theme.colors.primary200
        }
      />
    </span>
  )
    ...
    if (filterConf.type === 'select') {
      ...
      return (
        <CustomizedStatefulSelect
          ...
          initialState={{
            value: !inputFilter ? [] : [{id: inputFilter, label: inputFilter}],
          }}
          options={options}
          startOpen={true}
    ...
    <>
      <SortableHeadCell
        overrides={{
          ...
          HeadCell: {
            style: {...headerCellOverride.HeadCell.style, ...styleOverride},
          },
        }}
        title={title || ''}
