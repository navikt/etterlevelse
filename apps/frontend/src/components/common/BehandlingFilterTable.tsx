...
export const BehandlingFilterTable = (props: {
  filter: BehandlingFilters
  emptyText?: string
}) => {
  const {data, loading} = useQuery<{behandling: PageResponse<Behandling>}>(
    query,
    ...
    <Table
      ...
      config={{
        ...
        sorting: {
          ...
          overordnetFormaal: (a, b) =>
            a.overordnetFormaal.shortName.localeCompare(
              b.overordnetFormaal.shortName,
            ),
        ...
        return state.data.map((behnandling, i) => {
          return (
            <Row key={i}>
              ...
              <Cell>
                <RouteLink href={`/behandling/${behnandling.id}`}>
                  {behnandling.navn}
                </RouteLink>
              </Cell>
