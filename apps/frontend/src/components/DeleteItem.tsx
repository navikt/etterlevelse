  ...
  return (
    <>
      ...
      <Modal isOpen={open} onClose={() => setOpen(false)} unstable_ModalBackdropScroll>
        ...
        <ModalFooter>
          <Button onClick={() => setOpen(false)} size={'compact'} kind={'secondary'}>Avbryt</Button>
          <Button onClick={() => props.fun().then(() => history.push(props.redirect))} size={'compact'}>Slett</Button>
        </ModalFooter>
      </Modal>
    </>
  )
