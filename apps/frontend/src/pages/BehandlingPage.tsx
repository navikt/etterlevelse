      ...
      <Block paddingLeft="40px" paddingRight="40px" width="calc(100%-80px)">
        <Block>
          ...
          <Block display="flex" justifyContent="flex-end" marginBottom={theme.sizing.scale600}>
            ...
            {user.canWrite() && (
              <Button size="compact" kind={edit ? 'secondary' : 'primary'} onClick={() => setEdit(!edit)} marginLeft>
                {edit ? 'Avbryt' : 'Rediger'}
              </Button>
            )}
            {edit && (
              <Button size="compact" onClick={() => !formRef.current?.isSubmitting && formRef.current?.submitForm()} marginLeft>
                Lagre
              </Button>
            )}
          </Block>
