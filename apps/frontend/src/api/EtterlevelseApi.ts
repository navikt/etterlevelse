...
export const getEtterlevelsePage = async (
  pageNumber: number,
  pageSize: number,
) => {
  return (
    await axios.get<PageResponse<Etterlevelse>>(
...
export const getEtterlevelse = async (id: string) => {
  return (
    await axios.get<Etterlevelse>(`${env.backendBaseUrl}/etterlevelse/${id}`)
  ).data
}

export const deleteEtterlevelse = async (id: string) => {
  return (
    await axios.delete<Etterlevelse>(`${env.backendBaseUrl}/etterlevelse/${id}`)
  ).data
}
...
export const createEtterlevelse = async (etterlevelse: Etterlevelse) => {
  ...
  return (
    await axios.post<Etterlevelse>(`${env.backendBaseUrl}/etterlevelse`, dto)
  ).data
}
...
export const updateEtterlevelse = async (etterlevelse: Etterlevelse) => {
  const dto = etterlevelseToEtterlevelseDto(etterlevelse)
  return (
    await axios.put<Etterlevelse>(
      `${env.backendBaseUrl}/etterlevelse/${etterlevelse.id}`,
      dto,
    )
  ).data
}
