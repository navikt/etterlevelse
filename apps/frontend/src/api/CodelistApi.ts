...
export const getCodelistUsage = async (listname: ListName, code: string) => {
  return (
    await axios.get<CodeUsage>(
      `${env.backendBaseUrl}/codelist/usage/find/${listname}/${code}`,
    )
  ).data
}
...
export const replaceCodelistUsage = async (
  ...
  return (
    await axios.post<CodeUsage>(
      `${env.backendBaseUrl}/codelist/usage/replace`,
      {list: listname, oldCode, newCode},
    )
  ).data
}
