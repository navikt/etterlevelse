...
export const getBegrep = async (begrepId: string) => {
    return (await axios.get<Begrep>(`${env.pollyBaseUrl}/begrep/${begrepId}`)).data
}

export const searchBegrep = async (begrepSearch: string) => {
    return (await axios.get<PageResponse<Begrep>>(`${env.pollyBaseUrl}/begrep/search/${begrepSearch}`)).data.content
}
