...
export const getResourceById = async (resourceId: string) => {
  return (
    await axios.get<TeamResource>(
      `${env.backendBaseUrl}/team/resource/${resourceId}`,
    )
  ).data
}
...
export const searchResourceByName = async (resourceName: string) => {
  return (
    await axios.get<PageResponse<TeamResource>>(
...
export const getTeam = async (teamId: string) => {
  const data = (await axios.get<Team>(`${env.backendBaseUrl}/team/${teamId}`))
    .data
...
export const myTeams = async () => {
  return (
    await axios.get<PageResponse<Team>>(
      `${env.backendBaseUrl}/team?myTeams=true`,
    )
  ).data.content
}
...
export const searchTeam = async (teamSearch: string) => {
  return (
    await axios.get<PageResponse<Team>>(
      `${env.backendBaseUrl}/team/search/${teamSearch}`,
    )
  ).data
}
...
export const getSlackChannelById = async (id: string) => {
  return (
    await axios.get<SlackChannel>(
      `${env.backendBaseUrl}/team/slack/channel/${id}`,
    )
  ).data
}
...
export const getSlackUserByEmail = async (id: string) => {
  return (
    await axios.get<SlackUser>(
      `${env.backendBaseUrl}/team/slack/user/email/${id}`,
    )
  ).data
}
...
export const getSlackUserById = async (id: string) => {
  return (
    await axios.get<SlackUser>(`${env.backendBaseUrl}/team/slack/user/id/${id}`)
  ).data
}

export const searchSlackChannel = async (name: string) => {
  return (
    await axios.get<PageResponse<SlackChannel>>(
      `${env.backendBaseUrl}/team/slack/channel/search/${name}`,
    )
  ).data.content
}

export const mapTeamResourceToOption = (teamResource: TeamResource) => ({
...
const people: Map<string, {f: boolean; v: string}> = new Map<
  string,
  {f: boolean; v: string}
>()
const teams: Map<string, {f: boolean; v: Team}> = new Map<
  string,
  {f: boolean; v: Team}
>()
...
const tSubscribe = (id: string, done: () => void) => {
  !teams.has(id) &&
    teams.set(id, {
      f: false,
      v: {id, name: id, description: '', members: [], tags: []},
    })
  tsubs.has(id)
